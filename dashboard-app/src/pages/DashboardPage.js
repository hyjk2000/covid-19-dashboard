        ...
        onVizStateChanged={(newVizState) => {
          updateDashboardItem({
            variables: {
              id: item.id,
              input: {
                vizState: JSON.stringify(newVizState),
              },
            },
          });
        }}
        render={({ timeDimensions, updateTimeDimensions }) => (
          <DashboardItem
            key={item.id}
            itemId={item.id}
            title={item.name}
            ...
            onUpdateDateRange={(dateRange) =>
              timeDimensions.forEach((m) =>
                updateTimeDimensions.update(m, { ...m, dateRange })
              )
            }
          >
